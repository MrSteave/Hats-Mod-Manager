::CODE START
@ECHO off
title "Hat's Mod Manager"

::START OF ELEVATION PROTOCOL
:init
...
::Self update check
reg query HKCU\Software\HMM 1>nul 2>nul
if '%errorlevel%' == '0' ( goto VCheck )
reg add HKCU\Software\HMM >NUL
reg add HKCU\Software\HMM\ /v "Version" /t REG_SZ /d "0.0.1" >nul
:VCheck
::bitsadmin download version file
reg 

:Main
echo -= Welcome to the Hat's Mod Manager! =-
echo.
echo.
echo.
echo This program will install mods and files to a non-standard location to avoid conflicts
echo with other Minecraft versions. This means you will need to use a custom launcher profile
echo installed by this program.
echo.
echo.
